...
Example:
    python -m dogvision.tools.train_posture dataset.npz --out models/posture_model.joblib
    python -m dogvision.tools.train_posture dataset.npz --model mlp
"""
    ...
    if kind == "mlp":
        return Pipeline(
            [
                ...
                ("mlp", MLPClassifier(hidden_layer_sizes=(128, 64), max_iter=600, random_state=0)),
            ]
        )
...
def train(
    dataset_path: Path,
    model_kind: str = "rf",
    out_path: Path = Path("models/posture_model.joblib"),
    test_frac: float = 0.25,
    cv_folds: int = 5,
) -> Path:
    """Train, evaluate, and save a posture model bundle. Returns the bundle path."""
    ...
    if n_groups < cv_folds + 1:
        raise SystemExit(
            f"Only {n_groups} clips; need more for a meaningful split. "
    ...
    for tr, va in gkf.split(X, y, groups):
        m = build_model(model_kind)
        m.fit(X[tr], y[tr])
        accs.append(m.score(X[va], y[va]))
    print(f"\nGrouped {folds}-fold CV accuracy: {np.mean(accs):.3f} +/- {np.std(accs):.3f}")
    ...
    for c, rowv in zip(labels_sorted, cm, strict=True):
    ...
    print(
        f"Use it with:  python -m dogvision.tools.classify_video <video> --posture-model {out_path}"
    )
    return out_path
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("models/posture_model.joblib"))
