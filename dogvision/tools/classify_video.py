...
Examples:
    ...
    python -m dogvision.tools.classify_video samples/Video1.mp4

    # Explicit predictions path
    python -m dogvision.tools.classify_video samples/Video1.mp4 --predictions output/Video1.h5

    # Print which keypoint names DLC actually wrote
    python -m dogvision.tools.classify_video samples/Video1.mp4 --list-keypoints

    # Show debug overlay (per-feature numeric values)
    python -m dogvision.tools.classify_video samples/Video1.mp4 --debug

    # Disable keypoint smoothing (e.g. to compare jitter before/after)
    python -m dogvision.tools.classify_video samples/Video1.mp4 --no-smooth-keypoints
"""
...
from dogvision.orientation import OrientationSmoother, estimate_orientation
...
def find_predictions_h5(video: Path, predictions_dir: Path) -> Path | None:
...
def main() -> None:
    ...
    posture_clf: LearnedPostureClassifier | None = None
    if args.posture_model is not None:
        ...
        if abs(posture_clf.confidence_threshold - args.confidence) > 1e-6:
            print(
                f"  using model's training confidence threshold "
    ...
    if n_video_frames != len(raw_frames):
        print(
            f"  warning: video has {n_video_frames} frames but predictions has "
    ...
    if args.rerender:
        ...
        per_frame_labels = [(r["posture_label"], r["posture_score"]) for r in rows]
    else:
        ...
        if args.min_segment > 0:
            ...
            per_frame_labels = [(merged[i], ps) for i, (_pl, ps) in enumerate(per_frame_labels)]

        labels_cache.write_text(
            json.dumps([{"posture_label": pl, "posture_score": ps} for pl, ps in per_frame_labels])
        )
    ...
    # strict=False: in --rerender mode a stale labels cache may be shorter or
    # longer than the current frame list; render what lines up.
    for i, (frame, (posture_label, posture_score)) in enumerate(
        zip(smoothed_frames, per_frame_labels, strict=False)
    ):
